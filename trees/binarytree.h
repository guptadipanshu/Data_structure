...
class BinaryTree
{
	...
	Node * reverse(Node * node);
         ...
         *  as examples.
         */
	void findpaths(const Node * root,vector<T> myvector)	const;
	void inorder(const Node * root, T ele, T max,int flag)	const;
	bool check_bst(const Node *node, int min, int max) const;    
	void findSum(const Node * root,vector<T> myvector,int & sum)	const  ;  
	/**
