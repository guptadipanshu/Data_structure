...
{
    /* Your code goes here! */
	auto lookup=users.find(username);
	if(lookup==users.end())
    		return INVALID_USERNAME;
    		
		if(lookup->second.status==User::ONLINE)
			return FAIL;
		users.erase (lookup);
	 	return SUCCESS;
...
int Game::logout_user(const string& username)
{
    ...
    {
    	if(lookup->second.status==User::IN_GAME)
   		return USER_IN_MATCH;
   	lookup->second.status=User::OFFLINE;
    	return SUCCESS;
    }
 ...
 * (the paty id was invalid or a user in the party is already in a match).
  User s involved must exist, be online, and not be in another Match presentl
 * @param party_id The party id of the party creating the match.
 */ 
unsigned long Game::make_match(const string& party_id)
{
    /* Your code goes here! */
   static unsigned long match_id=10;
    auto lookup=parties.find(party_id);
    if(lookup==parties.end())
    	return NULL_MATCH_ID;
    for(auto& lookup2 : (lookup->second).members)
    	{
    		auto check = users.find(lookup2);
           	 if(check != users.end())
           	 {
            		if((check->second).status == User::IN_GAME)
            		return NULL_MATCH_ID;
   ...
   for(auto& lookup2 : (lookup->second).members)
    {
        ((users.find(lookup2))->second).match_id = match_id;
        ((users.find(lookup2))->second).status = User::IN_GAME;
    }
        // if(lookup->second.members==Match::IN_GAME)
    	//	return NULL_MATCH_ID;
    Match temp;
    temp.add_party(lookup->second);
    matches.insert(std::make_pair<>(match_id, temp));
	return     match_id++;	 	
...
{
    /* Your code goes here! */
    /*vector<string>::const_iterator it = usernames.begin();
    string key="";bool flag=true;
    for(it=usernames.begin();it!=usernames.end();it++)
    	strcat(key,*it);
    for(it=usernames.begin();it!=usernames.end();it++)
    {
    	auto lookup=users.find(*it);
    	if(lookup!=users.end())
    	{
    		if(lookup->second.status==User::ONLINE)
   		{
   			auto lookup2=parties.find(key);
   			if(lookup2!=parties.end())
   				flag=false;
   		}
    	}
    	else
    ...
    if(flag==true)
    	{
    		parties[key]=usernames;	
    		return key;
    	}
    	*/
    	 string key;
    for(auto& lookup : usernames)
    {
        auto lookup2 = users.find(lookup);
        if(lookup2 != users.end())
        {
            key = lookup;
            
            if((lookup2->second).status != User::ONLINE)
                return "";
            if((lookup2->second).party_id != "")
                return "";
        }
        else return "";
    }
    for(auto& lookup : usernames)
        (users.find(lookup))->second.party_id = key;
     Party temp;
    temp.members = usernames;
     parties.insert(std::make_pair<>(key,temp));
    return key;
    //return "";
}
...
int Game::get_user_stats(const string& username, UserStats& us)
{
    ...
    if(lookup != users.end())
    {
        us.wins = (lookup->second).us.wins;
        us.losses = (lookup->second).us.losses;
        us.playtime = (lookup->second).us.playtime;
        us.experience = (lookup->second).us.experience;
        us.win_streak = (lookup->second).us.win_streak;
